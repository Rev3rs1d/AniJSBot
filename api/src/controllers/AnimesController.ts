...
export const editAnime = async (req: Request, res: Response) => {
  ...
  const repo = getRepository(Animes)
  const filter =
    (req.user as Users).role === 'admininstrator'
      ? { id }
      : { id, userId: (req.user as Users).id }
  const anime = await repo.findOne(filter)

  if (!anime)
    return res.status(400).json({
      error:
        'Anime not registered in database or anime not was registered by you',
    })

  try {
    await repo.update(anime, {
      description,
      genre,
      name,
      year,
...
export const deleteAnime = async (req: Request, res: Response) => {
  ...
  try {
    await repo.remove(anime)

    res.json({ msg: 'anime deleted with success' })
  } catch ({ message }) {
    console.log(message)
